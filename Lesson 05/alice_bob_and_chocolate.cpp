 ...
 * Input:
 * The first line contains one integer n, the amount of bars on the table.
 * The second line contains a sequence t1, t2, ..., tn, where ti is the time (in seconds) needed to consume the i-th bar (in the order from left to right).
 * Output:
 * Print two numbers a and b, where a is the amount of bars consumed by Alice, and b is the amount of bars consumed by Bob.
 * SOURCE: http://codeforces.com/problemset/problem/6/C?locale=en
 *
 * SOLUTION
 * L'algoritmo mentre legge i tempi (bars[]) necessari a mangiare ciascuna barretta, ne calcola la somma.
 * Dopodiché "assegna" barrette ad Alice finché non raggiunge la metà, e infine aggiusta il caso Bob fa il true gentleman.
 * COMPLEXITY: O(n)
 */
